---
title: "Analysis code"
subtitle: "The role of memory in the inverse decision bias"
...
To this end, we assess memory accuracy in the Surprise Memory phase for Repetition vs Interference pairs
...
# Present model coefs
create_summary_coefs_memory <- function(M, Exp){
  ...
  summary_coefs <- draws_M %>%
    ...
    mutate(Exp = Exp) %>%
    rename(Coefficient = coef, Value = value, Experiment = Exp) %>%
    select(Experiment, Coefficient, Value)

  sum_coefs[[sprintf("M_memory_draws_%s",Exp)]] <- draws_M
  sum_coefs[[sprintf("summary_coefs_memory_%s",Exp)]] <- summary_coefs

  return(sum_coefs)
}

sum_coefs_memory_Exp1 <- create_summary_coefs_memory(M_memory_Exp1, "Exp1")
sum_coefs_memory_Pilot <- create_summary_coefs_memory(M_memory_Pilot, "Pilot")

# print table of coefs
bind_rows(sum_coefs_memory_Exp1$summary_coefs_memory_Exp1,sum_coefs_memory_Pilot$summary_coefs_memory_Pilot) %>%
...
Here we analyze the tendency to select a rewarded item in the Final Decisions phase as a function of choice (Chosen vs. Unchosen pairs) and condition (Repetition vs. Interference).  
...
Behavioral means and coefficient values can be found in the Supplementary Tables below.
...
# # print table of coefs
# bind_rows(sum_coefs_Exp1$summary_coefs_choice_delta_val_Exp1,sum_coefs_Pilot$summary_coefs_choice_delta_val_Pilot) %>%
#   kbl() %>%
#   kable_paper("hover", full_width = F, html_font = "Helvetica", position = "left")
...
We expect to observe this effect in unchosen pairs for Repeated items and not for Interfered items.  
...
We assess the coefficients of a multilevel Bayesian linear regression model predicting normalized RT (zscored_rt) as a function of choice (whether the selected item was rewarded or not, higher_outcome_chosen_centered) and condition (condition_center). We rearrange the model coefficients to get an estimate of the effect of choice on RT for the two conditions seperately (the slope term).  
Behavioral means and coefficient values can be found in the Supplementary Tables below.
...
RT_FD_group <- RT_FD %>%
  ...
  select(Exp, Choice, Condition, `Chosen item`, RT, `Normalized RT`) 
...
# =====================================================
# zscored rt as a function of gain choice and condition 
# =====================================================
...
if (run_models==1){
  ...
  load("../Data/Exp1/Models/M_zscored_RT_gain_cond_FD_Exp1.RData")
  load("../Data/Pilot/Models/M_zscored_RT_gain_cond_FD_Pilot.RData")
}
...
create_summary_coefs_rt_choice_model <- function(M, Exp){
  ...
  M_draws <- as.data.frame(M) %>%
    mutate(intercept_interference = `(Intercept)` + condition_center,
           slope_interference = higher_outcome_chosen_centered + `condition_center:higher_outcome_chosen_centered`,
           intercept_repetition = `(Intercept)` - condition_center,
           slope_repetition = higher_outcome_chosen_centered - `condition_center:higher_outcome_chosen_centered`,
  ...
  sum_coefs[[sprintf("M_zscored_RT_gain_cond_FD_draws_%s",Exp)]] <- M_draws
  sum_coefs[[sprintf("summary_coefs_rt_choice_%s",Exp)]] <- summary_coefs

  return(sum_coefs)
}

sum_coefs_rt_Exp1 <- create_summary_coefs_rt_choice_model(M_zscored_RT_gain_cond_FD_Exp1, "Exp1")
sum_coefs_rt_Pilot <- create_summary_coefs_rt_choice_model(M_zscored_RT_gain_cond_FD_Pilot, "Pilot")
...
print_list_rt <- print_model_coefs(bind_rows(sum_coefs_rt_Exp1$summary_coefs_rt_choice_Exp1,sum_coefs_rt_Pilot$summary_coefs_rt_choice_Pilot), c("(Intercept)","condition_center","higher_outcome_chosen_centered", "condition_center:higher_outcome_chosen_centered", "sigma"), c("Coefficient","Condition"))
...
#### Supplementary Text 1 - Analysis of Outcome Estimation phase
...
outcome_est_group_fits <- model_posterior[,!str_detect(colnames(model_posterior),"PID")] %>%
  mutate(interference_chosen_rewarded = `(Intercept)` - stim_type_centered + outcome + condition_centered - 
         ...
         interference_unchosen_unrewarded = `(Intercept)` + stim_type_centered - outcome + condition_centered + 
           `stim_type_centered:condition_centered` - `stim_type_centered:outcome` - `condition_centered:outcome` - 
           `stim_type_centered:condition_centered:outcome`,
         repetition_chosen_rewarded = `(Intercept)` - stim_type_centered + outcome - condition_centered + 
           `stim_type_centered:condition_centered` - `stim_type_centered:outcome` - `condition_centered:outcome` + 
           `stim_type_centered:condition_centered:outcome`,
         repetition_chosen_unrewarded = `(Intercept)` - stim_type_centered - outcome - condition_centered + 
           `stim_type_centered:condition_centered` + `stim_type_centered:outcome` + `condition_centered:outcome` - 
           `stim_type_centered:condition_centered:outcome`,
         repetition_unchosen_rewarded = `(Intercept)` + stim_type_centered + outcome - condition_centered - 
           `stim_type_centered:condition_centered` + `stim_type_centered:outcome` - `condition_centered:outcome` - 
           `stim_type_centered:condition_centered:outcome`,
         repetition_unchosen_unrewarded = `(Intercept)` + stim_type_centered - outcome - condition_centered - 
           `stim_type_centered:condition_centered` - `stim_type_centered:outcome` + `condition_centered:outcome` + 
           `stim_type_centered:condition_centered:outcome`) %>%
  gather(coef,value) %>%
  group_by(coef) %>%
...
#### Supplementary Text 2 - Analysis of inverse strategy
...
sum_coefs_memory_strategy_Exp1 <- create_summary_coefs_memory(M_memory_strategy_Exp1, "Exp1")
sum_coefs_memory_strategy_Pilot <- create_summary_coefs_memory(M_memory_Pilot_strategy, "Pilot")

# print table of coefs
bind_rows(sum_coefs_memory_strategy_Exp1$summary_coefs_memory_Exp1,sum_coefs_memory_strategy_Pilot$summary_coefs_memory_Pilot) %>%
...
# ========================================================
# zscored RT as a function of condition and p(choose gain)
# ========================================================
...
if (run_models==1){
  ...
  save(list = "M_zscored_RT_gain_cond_FD_strategy_Exp1",
       file = "../Data/Exp1/Models/Strategy/M_zscored_RT_gain_cond_FD_strategy_Exp1.RData")
  ...
  save(list = "M_zscored_RT_gain_cond_FD_strategy_Pilot",
       file = "../Data/Pilot/Models/Strategy/M_zscored_RT_gain_cond_FD_strategy_Pilot.RData")
} else {
  load("../Data/Exp1/Models/Strategy/M_zscored_RT_gain_cond_FD_strategy_Exp1.RData")
...
Here we present the tabled shown in the Supplementary Material.
